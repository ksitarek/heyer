...
export class ListItemComponent {
  ...
  public get minSalary(): number {
    var min = this.item.ContractsDetails.reduce((c1, c2) => {
      return c1.SalaryRange.IsPublished &&
        c1.SalaryRange.From < c2.SalaryRange.From
        ? c1
        : c2;
    });
  ...
  public get maxSalary(): number {
    var max = this.item.ContractsDetails.reduce((c1, c2) => {
      return c1.SalaryRange.IsPublished && c1.SalaryRange.To > c2.SalaryRange.To
        ? c1
        : c2;
    });
  ...
  public get isB2B(): boolean {
    return this.item.ContractsDetails.some(
      (c) => c.EmploymentType === EmploymentType.B2B
    );
  }

  public get isContractOfEmployment(): boolean {
    return this.item.ContractsDetails.some(
      (c) => c.EmploymentType === EmploymentType.ContractOfEmployment
    );
  }
